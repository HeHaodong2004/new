    ...
    def run_episode(self):
        ...
        for t in range(MAX_EPISODE_STEP):
            ...
            if is_fully_connected:
                ...
                if (self.assign_masks is None) or (self.rdv_point is None):
            ...
            for aid in range(N_AGENTS):
                if self.assign_masks is None:
                    region_empty[aid] = False
                    continue

                m = self.assign_masks[aid].astype(bool)
                has_goal = False
                for nd in self.node_manager.nodes_dict.__iter__():
                    if getattr(nd.data, "utility", 0.0) > 0:
                        # 用同一套 node_in_mask 逻辑（注意 global_map_info）
                        if self.robots[aid]._node_in_mask(nd.data.coords, global_map_info, m):
                            has_goal = True
                            break
                region_empty[aid] = (not has_goal)
