# worker.py  (full, repo-compatible, no-learning local planner + predicted-grid navigation)
...
class Worker:
    def __init__(self, meta_agent_id, policy_net, predictor, global_step,
        ...
        # Agent 不使用 policy_net 做 local planning
        self.robots = [
    ...
    def run_episode(self):
        ...
        for t in range(MAX_EPISODE_STEP):
            ...
            # ---------- if re-connected in the middle: reset old plan and repartition ----------
            if is_fully_connected and (not self.was_fully_connected):
                self.contract = None
                self.negotiated_plan = None
            ...
            # region_empty flags: no frontier inside assigned near region
            ...
            for i, r in enumerate(self.robots):
                ...
                r.set_assigned_mask(assign_mask_i)

                # current obs (for buffer compatibility)
                obs_i = r.get_observation(self.last_known_locations[i], self.last_known_intents[i])
                c_obs_i, _ = self.gtnm.get_ground_truth_observation(
                    r.location, r.pred_mean_map_info, self.env.robot_locations
                )
                r.save_observation(obs_i, self._match_intent_channels(c_obs_i))

                nxt_xy, nxt_idx, act_idx = r.select_next_waypoint(obs_i, greedy="heuristic")
                r.save_action(act_idx)
            ...
            # ---------- execute: if region empty -> FORCE go RDV immediately (predicted-grid nav) ----------
            picks = []
            for i, r in enumerate(self.robots):

                # 1) 区域探索完了：无 frontier -> 直接去 RDV
                if region_empty[i]:
                    if self.contract is not None and self.contract.status == 'active':
                        nxt_xy = self._forced_grid_to_region(
                            aid=i, agent=r,
                            center_xy=self.contract.P, radius_m=self.contract.r,
                            trav_mask=trav_mask, map_info=global_map_info
                        )
                        picks.append(nxt_xy)
                        continue
                    elif self.rdv_point is not None:
                        nxt_xy = self._grid_next_step_to_target(
                            aid=i, agent=r,
                            target_xy=self.rdv_point,
                            trav_mask=trav_mask, map_info=global_map_info
                        )
                        picks.append(nxt_xy)
                        continue
                ...
                # forced go to meeting region with predicted-grid nav
                nxt_xy = self._forced_grid_to_region(
                    aid=i, agent=r,
                    center_xy=self.contract.P, radius_m=self.contract.r,
                    trav_mask=trav_mask, map_info=global_map_info
    ...
    def resolve_conflicts(self, picks, dists):
        ...
        order = list(range(len(self.robots)))
    ...
    def _global_partition_and_rdv(self, global_map_info, trav_mask, unknown_mask, p_free, t_now):
        ...
        # ---- compute distance-to-nearest-agent for each cell ----
        ...
        # ---- partition near into N contiguous balanced regions ----
    ...
    def _balanced_region_growing_partition(self, near_mask, agent_rc):
        ...
        Also removes tiny disconnected crumbs.
        """
        H, W = near_mask.shape
        N = N_AGENTS
        ...
        tol = float(getattr(__import__('parameter'), "PARTITION_TOL", 0.10))  # 10% slack
        ...
        # ---- remove tiny disconnected components per region ----
        owner = self._remove_disconnected_crumbs(owner, near_mask)
    ...
    def _remove_disconnected_crumbs(self, owner, near_mask):
        """
        Keep largest connected component for each region.
        Reassign small crumbs to neighboring regions.
        """
        H, W = near_mask.shape
        N = N_AGENTS
        ...
        for aid in range(N):
            ...
            for comp in crumbs:
                for (r, c) in comp:
                    # reassign to best neighboring region
                    neigh = []
                    for dr, dc in [(0,1),(0,-1),(1,0),(-1,0)]:
                        nr, nc = r + dr, c + dc
                        if 0 <= nr < H and 0 <= nc < W and new_owner[nr, nc] != -1 and new_owner[nr, nc] != aid:
                            neigh.append(new_owner[nr, nc])
                    if neigh:
                        new_owner[r, c] = int(neigh[0])
                    else:
                        new_owner[r, c] = -1

        # any -1 leftover -> nearest assigned neighbor
        leftovers = np.argwhere(near_mask & (new_owner == -1))
        if leftovers.size > 0:
            for r, c in leftovers:
                neigh = []
                for dr, dc in [(0,1),(0,-1),(1,0),(-1,0)]:
                    nr, nc = r + dr, c + dc
                    if 0 <= nr < H and 0 <= nc < W and new_owner[nr, nc] != -1:
                        neigh.append(new_owner[nr, nc])
                if neigh:
                    new_owner[r, c] = int(neigh[0])
    ...
    def _pick_rdv_from_far(self, global_map_info, trav_mask, far_mask, unknown_mask, p_free, t_now):
        ...
        # multi-source dist maps on trav_mask
        dist_maps, starts_rc = [], []
    ...
    # ----------------------- Predicted Grid Forced Navigation -----------------------
    def _grid_next_step_to_target(self, aid, agent, target_xy, trav_mask, map_info):
        """
        Grid BFS on trav_mask, return one-step world xy toward target_xy.
        """
        H, W = trav_mask.shape
        cell_size = float(map_info.cell_size)

        start_rc = self._world_to_cell_rc(agent.location, map_info)
        goal_rc  = self._world_to_cell_rc(target_xy, map_info)

        if not (0 <= start_rc[0] < H and 0 <= start_rc[1] < W and trav_mask[start_rc]):
            start_rc = tuple(self._find_nearest_valid_cell(trav_mask, np.array(start_rc)))
        if not (0 <= goal_rc[0] < H and 0 <= goal_rc[1] < W and trav_mask[goal_rc]):
            goal_rc = tuple(self._find_nearest_valid_cell(trav_mask, np.array(goal_rc)))

        if start_rc == goal_rc:
            return agent.location.copy()

        dist = self._bfs_dist_map(trav_mask, tuple(start_rc))
        if not np.isfinite(dist[goal_rc]):
            return agent.location.copy()

        r0, c0 = start_rc
        best_rc = start_rc
        best_d = dist[start_rc]

        for dr, dc in [(0,1),(0,-1),(1,0),(-1,0)]:
            nr, nc = r0 + dr, c0 + dc
            if 0 <= nr < H and 0 <= nc < W and np.isfinite(dist[nr, nc]):
        ...
        nr, nc = best_rc
        nxt_xy = np.array([
            map_info.map_origin_x + nc * cell_size,
            map_info.map_origin_y + nr * cell_size
        ], dtype=float)
        return nxt_xy


    def _forced_grid_to_region(self, aid, agent, center_xy, radius_m, trav_mask, map_info):
        """
        Forced nav to a circular region using trav_mask grid BFS.
        """
        H, W = trav_mask.shape
        cell_size = float(map_info.cell_size)

        start_rc = self._world_to_cell_rc(agent.location, map_info)
        if not trav_mask[start_rc]:
            start_rc = tuple(self._find_nearest_valid_cell(trav_mask, np.array(start_rc)))

        dist = self._bfs_dist_map(trav_mask, tuple(start_rc))

        c_rc = self._world_to_cell_rc(center_xy, map_info)
        rad_pix = int(max(1, round(radius_m / cell_size)))

        best_goal = None
        best_d = float('inf')

        r0 = max(0, c_rc[0] - rad_pix); r1 = min(H, c_rc[0] + rad_pix + 1)
        c0 = max(0, c_rc[1] - rad_pix); c1 = min(W, c_rc[1] + rad_pix + 1)

        for rr in range(r0, r1):
            for cc in range(c0, c1):
                if not trav_mask[rr, cc]:
                    continue
                if (rr - c_rc[0])**2 + (cc - c_rc[1])**2 > rad_pix**2:
                    continue
                d = dist[rr, cc]
        ...
        # one-step descent on dist map
        r_start, c_start = start_rc
        best_rc = start_rc
        best_val = dist[start_rc]
    ...
    def plot_env(self, step):
        ...
        try:
            if (self.assign_masks is not None
                ...
                for aid in range(N_AGENTS):
                    ...
                    if m.sum() == 0:
                        continue
                    ax_global.contour(m.astype(np.float32), levels=[0.5],
                                      colors=[cmap_part(aid)],
                                      linewidths=2.6, origin='lower', zorder=4)

                    rr, cc = np.where(m)
                    cy, cx = float(np.mean(rr)), float(np.mean(cc))
                    ax_global.text(cx, cy, f"A{aid}",
        ...
        # trajectories / comm circles / positions
        for i, r in enumerate(self.robots):
            ...
            ax_global.plot(pos_cell[1], pos_cell[0], 'o', ms=11,
                           mfc=agent_colors(i), mec='white', mew=1.8, zorder=10)

        # RDV point (even if no contract)
