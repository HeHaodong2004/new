...
class Worker:
    ...
    def _update_plan_when_connected(self, global_info: MapInfo,
        ...
        self.far_uncertain_mask = far_mask

        self.assign_masks, _ = self._balanced_partition_near(near_mask, global_info)
        cand = self._select_rdv_in_far(
            far_mask, global_info, trav_mask, unknown_mask, p_free, t_now
        )
    ...
    def plot_env(self, step):
        ...
        # dim the base map so overlays pop
        ax_global.imshow(global_info.map, cmap='gray',
                        origin='lower', alpha=0.55)
        ax_global.set_aspect('equal', adjustable='box')
        ax_global.set_axis_off()

        # ----------------- FAR (weak prediction zone) background -----------------
        try:
            if hasattr(self, "far_uncertain_mask") and self.far_uncertain_mask is not None:
                far_mask = self.far_uncertain_mask.astype(bool)
                far_rgba = np.zeros((H, W, 4), dtype=np.float32)
                far_rgba[far_mask] = np.array([0.0, 1.0, 1.0, 0.18], dtype=np.float32)  # cyan
        ...
        # ----------------- NEAR partitions (strong overlay) -----------------
        try:
            if (self.assign_masks is not None
                ...
                # make region colors visible
                colored[..., 3] = 0.45
                ax_global.imshow(colored, origin='lower', zorder=2)

                # thick colored contours + big labels
                for aid in range(N_AGENTS):
                    m = self.assign_masks[aid].astype(bool)
                    if m.sum() == 0:
                        continue

                    # boundary contour (agent color, thick)
                    ax_global.contour(
                        m.astype(np.float32),
                        levels=[0.5],
                ...
                # fallback if assign_masks missing
